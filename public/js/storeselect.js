...
const city1upt = document.querySelectorAll(".city1upt");
const city2upt = document.querySelectorAll(".city2upt");

taggener(city1);
city1upt.forEach(function(item,index){
    taggener(item);
    item.addEventListener("change",function(){
        //선택한 option 태그의 data-sido의 값을 가지고 옴
    let city1Data = item.options[item.selectedIndex].getAttribute("data-sido");
    
    // 구 / 군 만드는 코드 함수로 만들어서 호출
    sigugun(city1Data,city2upt[index]);
    });
})


function taggener(para){
...
city1.addEventListener("change",function(){
    ...
    // 구 / 군 만드는 코드 함수로 만들어서 호출
    sigugun(city1Data,city2);
});


//시/구/군 생성하는 코드작업 함수

function sigugun(city1Data,para2){
    //기존 option 태그들 갈아치워주자
    para2.innerHTML = "<option value>구/군 선택</option>";
